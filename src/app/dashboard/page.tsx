    ...
    return (
        <section className="min-h-screen bg-[hsl(220,13%,98%)]/50 font-inter relative">
            ...
            <header className="sticky top-0 bg-white border-b border-[hsl(220,13%,91%)]">
                <div className="max-w-6xl mx-auto px-4 sm:px-16 h-14 flex items-center justify-between">
                    {/* Left side */}
                    <div className="flex items-center gap-4">
                        <Button
                            ...
                            iconPosition="left"
                            onClick={() => router.push("/")}
                            className="hover:bg-yellow-300 group flex"
                        >
                            <span className="text-gray-500 font-medium group-hover:text-gray-800 hidden sm:block">Back to Store</span>
                        </Button>
                        <div className="w-px h-5 bg-[hsl(220,13%,91%)] hidden sm:block" />
            ...
            <main className="max-w-6xl mx-auto px-6 py-8">
                ...
                { isOrderTab && (
                    <div className="bg-[hsl(0,0%,100%)] rounded-lg border border-[hsl(220,13%,91%)]">
                        ...
                        {mockOrders.map((i) => (
                            <div key={i.id} className="px-5 py-4 md:flex md:items-center md:justify-between border-b border-[hsl(220,13%,91%)]">
                                <div className="*:py-1">
                                    <div className="flex items-center justify-between">
                                        <p className="md:text-sm font-medium text-gray-700">{i.id.slice(0,13)}<span className="text-gray-500 px-2 text-xs md:hidden">{i.items} {i.items > 1 ? "items" : "item"}</span></p>
                                        <p className="text-sm text-gray-700 font-semibold md:hidden">${i.total}</p>
                                    </div>
                                    <p className="text-sm text-gray-500">{i.customer}</p>
                                    <p className="text-xs text-gray-500 md:hidden">{i.date}</p>
                                </div>
                                <p className="text-sm text-gray-500 hidden md:block">{i.items} {i.items > 1 ? "items" : "item"}</p>
                                <p className="text-sm text-gray-500 hidden md:block">{i.date}</p>
                                <div className={`${i.colors} md:w-28 rounded-full flex items-center justify-center gap-0.5 p-1 text-xs`}>
                                    <i.icon size={14} />
                                    {i.status}
                                </div>
                                <p className="text-sm text-gray-700 font-semibold hidden md:block">${i.total}</p>
                            </div>
                        ))}
                    </div>
                )}
                    ...
                    <>
                        <div className="bg-[hsl(0,0%,100%)] rounded-lg border border-[hsl(220,13%,91%)]">
                            ...
                            .map((i) => {
                                ...
                                return (
                                <div key={product.id} className="px-5 py-4 grid grid-cols-[1fr_auto] md:grid-cols-3 md:items-center gap-2 md:gap-0 justify-between border-b border-[hsl(220,13%,91%)]">
                                    <div className="flex items-center gap-1.5">
                                        <Image className="w-14 h-14 md:w-10 md:h-10 object-contain" src={product.thumbnail} width={40} height={40} alt="product-image" />
                                        <div className="*:py-0.5">
                                            <p className="md:text-sm text-gray-700">{product.title}</p>
                                            <p className="text-sm md:text-xs text-gray-400">{product.brand}<span className="text-xs text-gray-500 md:hidden">{product.brand ? " / " : ""}{product.category}</span></p>
                                            <div className="flex items-center gap-2 md:hidden">
                                                <p className="text-sm text-gray-800 font-medium md:hidden">${discountPrice(product.price, product.discountPercentage)}</p>
                                    ...
                                    <p className="text-sm text-gray-500 text-center hidden md:block">{product.category}</p>
                                    <div className="flex items-end justify-end gap-10">
                                        <p className="text-sm text-gray-800 font-medium hidden md:block">${discountPrice(product.price, product.discountPercentage)}</p>
                                        ...
                                        <div className="flex items-center justify-end gap-2">
                                            <Button
                                                ...
                                                className="hover:bg-black/10 px-1 py-0.5 rounded-md"
                                            >
                            ...
                            <AnimatePresence>
                                {isDeleteModalOpen && (
                                    ...
                                    >
                                        ...
                                        >
                                            <div className="bg-white p-5 rounded-2xl w-96 md:w-[500px] border border-[hsl(220,13%,91%)]">
